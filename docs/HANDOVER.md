...
## 五、M5 它是你创造的（下一个里程碑）——范围已在 spec §13 钉死

SVG 形象系统：基因组 + 阶段骨架 + 表情变体 + 部件级动画 + `SVGRenderer`（透明 WKWebView v0）+ 共创仪式 + 初见孵化 onboarding（蛋 → 创造仪式完成即孵化）；位图生图作可选换装来源。形象穿刺已验证 `spikes/svg-pet/`
