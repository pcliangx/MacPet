...
| M7 实现计划 | `docs/superpowers/plans/2026-06-14-m7-has-friends.md` |
| M8 实现计划 | `docs/superpowers/plans/2026-06-14-m8-plaza-ladder.md` |
...
- **M7 它有朋友了 ✅ 已交付**：tag `v0.8.0-m7`，226 单测全绿（+28 新增）
  - PetIdentity（密钥对+名片+签名，孵化即领）
  - FriendTicket（base64 编解码+验证）+ FriendStore（好友/宿敌/战绩）
  - CourierProtocol（Swift↔courier NDJSON 消息族）
  - BattleEngine（确定性数值底盘+SeededRNG+性格加成+演出叙事）
  - 档案 v2（含身份密钥+好友，v1 容忍导入）
  - 注：iroh Rust courier 为独立 crate，Swift 侧协议就绪，网络互联留产品化
- **M8 广场与朋友圈天梯 ✅ 已交付**：tag `v0.9.0-m8`，252 单测全绿（+26 新增）
  - PlazaGossip（见闻+注入防御 sanitize+故事包装）
  - SocialSafety（拉黑/举报/仅好友模式/社交总开关）
  - FriendLadder（胜率×log(场次) 排名，成年门控）
  - BadgeCollection（6 种徽章+图鉴）
  - DaemonSoul 阶段门控（少年串门/成年广场天梯）
- **M9 ⬜ 未开始**。下一步 = M9（最后一个里程碑）。
...
## 五、M9 它收到了礼物（最后一个里程碑）——范围已在 spec §13 钉死

插件标准**对外开放**：第三方文档 + 权限确认 UI 完备 + **MCP 桥** + 礼物仪式 + 示例插件 ×2（一个 sense、一个 tool）
